...
export default function UsersList() {
      /* ---------- hook ---------- */
    const {data: users,isLoading} = useSWR<Array<User>>("users", () => {
    ...
    if(isLoading) {
        return (
            <>
                {
                    Array.from({length: 5}).fill(undefined).map((_,idx) => (<div key={idx} className="flex items-center space-x-4">
    ...
    if(!users?.length) {
        return (
            <div className="flex-center flex-col pt-10">
                ...
                <p className="text-muted-foreground text-lg font-semibold">You are alone</p>
            </div>
        )
    }
